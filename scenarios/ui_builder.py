...
class ScenarioUIBuilder:
    ...
    def build_panel(self):
        ...
        self.ui.btn_back_to_phase1.setStyleSheet("background-color: #607D8B; color: white; font-weight: bold; height: 35px;")
    ...
    def _build_standard_scenario_ui(self, tab_widget, scenario_name):
        ...
        content_widget = QWidget(); l = QVBoxLayout(content_widget)
        ...
        hl1 = QHBoxLayout()
        btn_lock = QPushButton("配置强制锁定海域..."); controls['btn_lock'] = btn_lock
        lbl_lock_status = QLabel("已锁定: 0 项"); lbl_lock_status.setStyleSheet("color: gray;"); controls['lbl_lock_status'] = lbl_lock_status
        hl1.addWidget(btn_lock); hl1.addWidget(lbl_lock_status); gb_lock.setLayout(hl1); l.addWidget(gb_lock)

        # 2. 分类海域目标配额
        gb_target = QGroupBox("🎯 全局分区约束占比 (%)")
        gl = QGridLayout()
        categories = {
            "🟢 保育类(ConsRes/TZE)": ['Z1_MPZ', 'Z5_TZE', 'Z10_RZ'],
            "🔵 社区与渔业(CmmExtr/Fshr)": ['Z6_FZT', 'Z7_FZC', 'Z8_FZA'],
            "🟠 开发与矿运(UIZ/PSZ/Mining)": ['Z2_UIZ', 'Z3_PSZ', 'Z4_TZ', 'Z9_SPZ']
        }
        ...
        for cat_name, zones in categories.items():
            ...
            for z in zones:
                ...
                # 根据不同情景配置默认保留百分比
                if "保持现状" in scenario_name: spin.setValue(15 if "UIZ" in z else 10)
                elif "均衡发展" in scenario_name: spin.setValue(30 if "MPZ" in z else 8)
                elif "保护优先" in scenario_name: spin.setValue(45 if "MPZ" in z else 5)
                elif "开发优先" in scenario_name: spin.setValue(25 if "UIZ" in z else 8)
                else: spin.setValue(10)
        ...
        gb_target.setLayout(gl); l.addWidget(gb_target)
        ...
        btn_run.setStyleSheet("background-color: #E91E63; color: white; font-weight: bold; height: 45px;")
