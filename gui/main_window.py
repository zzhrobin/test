...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.study_area = None; self.final_grid = None; self.feature_layers = []
        self.bay_area = None; self.villages = None; self.custom_conflict_matrix = None
    ...
    def _init_global_params(self):
        default_params = {
            "time_decay_max": 100.0, "time_decay_min": 20.0, "fishery_range_km": 15.0, "holness_base_cost": 0.3,
            "influence_strong_m": 1500.0, "influence_weak_m": 500.0, "sci_sigma_short": 3.0, "sci_sigma_long": 10.0,
        }
        if os.path.exists(self.config_file):
            try:
                with open(self.config_file, "r", encoding="utf-8") as file: self.global_params = json.load(file)
            except Exception: self.global_params = default_params
        else:
            self.global_params = default_params
            with open(self.config_file, "w", encoding="utf-8") as file: json.dump(default_params, file, indent=4, ensure_ascii=False)

    def init_ui(self):
        main_widget = QWidget(); self.setCentralWidget(main_widget); layout = QHBoxLayout(main_widget)
        # 获取最新的 StackedWidget
        left_panel = self.ui.create_left_panel(); layout.addWidget(left_panel)
        self.visual_layout = QVBoxLayout(); layout.addLayout(self.visual_layout, 3) 
    ...
    def connect_signals(self):
        ...
        self.ui.btn_load_normal.clicked.connect(lambda: self.handlers.load_feature(is_ecological=False))
        self.ui.btn_load_eco.clicked.connect(lambda: self.handlers.load_feature(is_ecological=True))
        self.ui.btn_delete_feature.clicked.connect(self.handlers.delete_selected_feature)
        ...
        self.ui.btn_run_fishery_cost.clicked.connect(self.handlers.run_fishery_cost_calculation)
        self.ui.btn_edit_matrix.clicked.connect(self.handlers.open_matrix_editor)
        self.ui.btn_run_conflict.clicked.connect(self.handlers.run_conflict_calculation)
        self.ui.btn_run_total_cost.clicked.connect(self.handlers.run_total_cost_calculation)
        self.ui.btn_global_params.clicked.connect(self.handlers.open_global_params_editor)
        # 把预检按钮和后台弹窗函数连起来
        self.ui.btn_confirm_roles.clicked.connect(self.handlers.open_role_confirmation_dialog)