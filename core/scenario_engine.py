...
def resolve_scenario_allocation(grid_gdf: gpd.GeoDataFrame, scenario_name: str, confirmed_mapping: dict, locked_features: list, zone_targets: dict, special_targets: dict, global_params: dict, custom_matrix: dict) -> tuple:
    ...
    logs.append(f"🔍 剔除锁定极值后，提取全域本底落差: Min={global_min:.2f}, Max={global_max:.2f}")
    
    cost_matrix = np.zeros((total_cells, num_zones))
    norm_costs = {}
    for idx, z in enumerate(ZONES_10):
        col_name = f'Cost_L_{z}'
        if global_max > global_min: norm_array = ((grid_gdf[col_name].values - global_min) / (global_max - global_min)) * 100.0 + 1.0
        else: norm_array = np.ones(total_cells)
        norm_costs[z] = norm_array
        grid_gdf[f'Norm_Cost_{z}'] = norm_array

    # 边界集聚平滑 (模拟 BLM)
    max_r = grid_gdf['row_idx'].max(); max_c = grid_gdf['col_idx'].max()
    for idx, z in enumerate(ZONES_10):
        ...
        blended = 0.5 * norm_costs[z] + 0.5 * smoothed_2d[grid_gdf['row_idx'], grid_gdf['col_idx']]
    ...
    for role, cols in confirmed_mapping.items():
        if role in ZONES_10: zone_features[role].extend(cols)
        elif role == 'L4_交通区(港口码头)': zone_features['Z3_PSZ'].extend(cols)
        elif role == 'L1_工业矿业能源区': zone_features['Z2_UIZ'].extend(cols)
        elif '生态' in role: zone_features['Z1_MPZ'].extend(cols)
        elif '渔业' in role: zone_features['Z6_FZT'].extend(cols)
    ...
    for z in ZONES_10:
        ...
        if quota > 0:
            logs.append(f"🎯 [{z.split('_')[1]}] 现有足迹 {footprint_area} 个网格，提取目标配额: {quota} 个")
    ...
    if g_solution is not None:
        final_zones = []
        for i in g_solution:
            if i == -1: final_zones.append("Z10_RZ") # 核心：Gurobi没分配的留白网格，自动归入保留区！
            else: final_zones.append(ZONES_10[i])
